...
- A small per-player read-out lives in `player.gui.top` (a sibling flow alongside the hive buttons) showing `[item=hm-pollution] N` where N is the pollution capacity of the network covering the player's first hive. Refreshed every `shared.intervals.labels` ticks, recoloured green / amber / red by health. Per-player GUI elements only sync to the owning client, so the read-out doesn't generate cross-client packet traffic the way the previous in-world render-object labels did. The earlier per-hive floating label is gone — render-object updates were a measurable MP cost (every label write was broadcast to every client every tick window) and the visible clutter on the hive entity was unwelcome anyway.
- Cap recomputation is dirty-checked: the previous value is stored on the player's GUI state; we only call `Network.hives_for_position` + `Cost.pollution_capacity` when the watchdog ticks. The per-player iteration replaces the old per-hive iteration, so for a single player with one hive the label tick now does one network resolution per cadence instead of one per hive.
...
- Disgorge: when a burst is created, the recruitment scan triggers a one-shot disgorge across the surface (gated by `Pheromone.consume_disgorge_flag`). `disgorge_hive_units` releases stored creatures from the network's primary chest as live units commanded to the burst position, **bounded by `shared.pheromone_burst.disgorge_cap_factor`** (default `3`, applied as `cap = target_size × disgorge_cap_factor`) so a chest with thousands of stored creatures doesn't lock the game with an unbounded `find_non_colliding_position` loop. Excess creatures stay in storage for the next burst. Pollution stays in storage regardless.
