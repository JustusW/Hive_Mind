...
local set_hive_build_cursor = function(player, entity_name)
...
local on_gui_click = function(event)
  ...
  if name and name:find(build_button_prefix, 1, true) == 1 then
    ...
    set_hive_build_cursor(player, entity_name)
    return
  end
...
local set_hive_cursor_item = function(player, stack)
  update_selected_build_label(player, stack.name)
end

local get_selected_hive_build_name = function(player)
  local selected = player.selected
  if not (selected and selected.valid) then return end
  if allowed_cursor_items[selected.name] then
    return selected.name
  end
  if selected.name and selected.name:find(names.growth_node_prefix, 1, true) == 1 then
    local target_name = selected.name:sub(#names.growth_node_prefix + 1)
    if allowed_cursor_items[target_name] then
      return target_name
    end
  end
end

local on_player_pipette = function(event)
  local player = game.get_player(event.player_index)
  if not (player and player.valid) then return end
  if not is_hivemind_force(player.force) then return end

  local selected_name = get_selected_hive_build_name(player)
  if selected_name then
    set_hive_build_cursor(player, selected_name)
  end
end
...
local on_player_cursor_stack_changed = function(event)
  ...
  if not stack.valid_for_read then
    local selected_name = get_selected_hive_build_name(player)
    if selected_name then
      stack.set_stack{name = selected_name, count = 1}
      update_selected_build_label(player, selected_name)
    else
      update_selected_build_label(player, nil)
    end
    return
  ...
  if allowed_cursor_items[stack.name] then
    set_hive_cursor_item(player, stack)
    return
  end
