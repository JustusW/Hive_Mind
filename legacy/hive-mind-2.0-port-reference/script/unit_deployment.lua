...
local get_ghost_target_name = function(ghost_data)
  return ghost_data.target_name or get_growth_node_target_name(ghost_data.entity) or nil
end
...
local restore_hive_build_cursor = function(player_index, entity_name)
  if not player_index then return end
  local player = game.get_player(player_index)
  if not (player and player.valid) then return end
  local stack = player.cursor_stack
  if stack.valid_for_read and stack.name ~= entity_name then
    return
  end
  stack.set_stack{name = entity_name, count = 1}
end

local start_entity_construction = function(entity, player_index)
  local entity_name = entity.name

  if get_needs_technology(entity_name) and not entity.force.technologies["hivemind-unlock-"..entity_name].researched then
    if player_index then
      local player = game.get_player(player_index)
      player.create_local_flying_text
      {
        text={"entity-not-unlocked", get_prototype(entity_name).localised_name},
        position=entity.position,
        color=nil,
        time_to_live=nil,
        speed=nil
    ...
    entity.destroy()
    restore_hive_build_cursor(player_index, entity_name)
    return
  end

  if needs_creep[entity_name] and entity.surface.get_tile(entity.position).name ~= creep_name then
    if player_index then
      local player = game.get_player(player_index)
      player.create_local_flying_text
      {
        text={"must-be-placed-on-creep", get_prototype(entity_name).localised_name},
        position=entity.position,
        color=nil,
        time_to_live=nil,
        speed=nil
  ...
  local force = entity.force
  local direction = entity.direction
  local pollution = get_required_pollution(entity_name)
  entity.destroy()
  local growth_node = create_growth_node(surface, position, force, entity_name, pollution, direction)
...
local on_built_entity = function(event)
  ...
  if event.player_index and required_pollution[entity.name] then
    return start_entity_construction(entity, event.player_index)
  end
